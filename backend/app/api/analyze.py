...
async def analyze_content(request: Request, body: AnalyzeRequest) -> AnalyzeResponse:
    ...
    # ─── Cache check FIRST ───
    # `v3` namespace = bumped after the verifier-first 2-pass rewrite. Old v2
    # entries used the broken single-call architecture (perplexity-reasoning
    # buried in a JSON-rubric prompt → stale training-data verdicts). Bumping
    # invalidates them so users see the new accurate results immediately.
    # Date suffix ensures political/recency claims re-verify each day — facts
    # about current office-holders shouldn't be cached across days when the
    # underlying world can change overnight.
    content_hash = hashlib.sha256(content.encode()).hexdigest()[:16]
    today_key = datetime.now(timezone.utc).strftime("%Y%m%d")
    cache_key = f"trustlens:analysis:v3:{today_key}:{content_hash}"
